...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($action == 'cut' || $action == 'copy') {
        if (isset($_POST['selected'])) {
            ...
            header("Location: list.php?directory=" . urlencode($relativePath) . "&message=Items " . ($action == 'cut' ? 'cut' : 'copied'));
            exit;
        }
    } elseif ($action == 'paste') {
        if (isset($_SESSION['clipboard'])) {
            ...
            header("Location: list.php?directory=" . urlencode($relativePath) . "&message=Items pasted");
            exit;
        }
    } elseif ($action == 'cancel') {
        unset($_SESSION['clipboard']);
        unset($_SESSION['selected_items']);
        header("Location: list.php?directory=" . urlencode($relativePath) . "&message=Operation cancelled");
        exit;
    }
}
